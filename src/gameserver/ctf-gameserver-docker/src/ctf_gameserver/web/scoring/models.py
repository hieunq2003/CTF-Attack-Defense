...
class Penalty(models.Model):
    """
    Manual score adjustment for a team.
    Value < 0  => trừ điểm
    Value > 0  => cộng thêm điểm (bonus / hoàn lại)
    """
    ...
    team = models.ForeignKey(
        ...
        help_text=_('Team gets points deducted / added.'),
    )
    service = models.ForeignKey(
        ...
        blank=True,
        related_name='penalties',
        help_text=_('If left blank, it will be applied to the team’s overall total score.'),
    )
    value = models.FloatField(
        help_text=_('Adjustment value. Use a negative number to deduct points, a positive number to add points.')
    )
    reason = models.CharField(
        max_length=255,
        blank=True,
        help_text=_('Reason for deduction/addition (displayed in admin).'),
    )
    is_active = models.BooleanField(
        default=True,
        help_text=_('Uncheck to temporarily disable the penalty without deleting the record.'),
    )
