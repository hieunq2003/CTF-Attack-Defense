...
def scores(select_related_fields=None, only_fields=None):
    ...
    # Thêm key 'penalty' để dễ debug / export
    ...
    # Áp dụng penalty (manual adjust)
    penalties_by_team = models.Penalty.objects.filter(is_active=True) \
                                              .values('team') \
                                              .annotate(total_penalty=Sum('value'))
    penalty_map = {row['team']: row['total_penalty'] for row in penalties_by_team}

    for team in team_scores.keys():
        penalty_value = penalty_map.get(team.pk, 0) or 0
        team_scores[team]['penalty'] = penalty_value
        team_scores[team]['total'] += penalty_value

    sorted_team_scores = OrderedDict(
        sorted(team_scores.items(), key=lambda s: s[1]['total'], reverse=True)
    )
